// 这将包含所有 image_block 的内容 --> 图像，图像上的文本
export const image_block = {
  image: "/assets/HomePage/HomeTitle.jpg",
  title: "顶级全方位服务律师事务所",
  text: "专业 - 国际化 - 足智多谋",
  color: "blue",
  textColor: "#00467f",
};

export const home_introduction = {
  title: "高质量的代理，拥有卓越的记录",
  content: `在马萨诸塞州及全球范围内，我们因前沿的法律策略、多样化的员工、协作的工作方式和取得积极结果的能力而闻名。我们的律师事务所由一支具有独特背景的律师团队组成，这使我们能够创造性地并成功地满足客户的需求。

我们提供房地产、遗产规划、商业法、移民法等方面的服务。如果您有法律需求，我们可以帮助您。

我们的主办公室位于马萨诸塞州莱克星顿，并在纽约、加利福尼亚和中国设有办事处。我们期待了解您的需求，帮助您实现目标。若要讨论您的案件并与我们知识渊博的员工交谈，今天就可以联系我们的事务所。`,
  image: "/assets/HomePage/HomeIntroduction.jpeg",
  color: "white",
};

export const practices = {
  title: "业务领域",
  law_types: [
    {
      id: 0,
      title: "移民法",
      subgroups: [
        "EB-5 移民",
        "L-1 签证",
        "EB-1A 签证",
        "H-1B 签证",
        "F-1 学生签证",
      ],
      icon: "/assets/HomePage/PracticesIcons/immigration.png",
    },
    {
      id: 1,
      title: "商业与合同法",
      subgroups: [
        "公司/有限责任公司设立",
        "并购",
        "雇佣协议",
        "股票期权",
        "保密协议",
      ],
      icon: "/assets/HomePage/PracticesIcons/company.png",
    },
    {
      id: 2,
      title: "房地产法",
      subgroups: [
        "购房协议审查",
        "房地产交割",
        "租赁协议",
        "分区与许可",
        "房地产信托",
      ],
      icon: "/assets/HomePage/PracticesIcons/real-estate.png",
    },
    {
      id: 3,
      title: "税务",
      subgroups: ["个人税", "公司税", "税务规划", "税务咨询", "会计"],
      icon: "/assets/HomePage/PracticesIcons/tax.png",
    },
    {
      id: 4,
      title: "遗产规划",
      subgroups: ["遗嘱和信托", "授权书", "遗产税规划", "遗产管理", "生前遗嘱"],
      icon: "/assets/HomePage/PracticesIcons/estate-planning.png",
    },
    {
      id: 5,
      title: "知识产权",
      subgroups: ["专利", "商标", "版权", "商业秘密", "知识产权保护规划"],
      icon: "/assets/HomePage/PracticesIcons/ip.png",
    },
    {
      id: 6,
      title: "民事/刑事诉讼",
      subgroups: [
        "人身伤害索赔",
        "合同违约",
        "商业纠纷",
        "家庭暴力",
        "白领犯罪",
      ],
      icon: "/assets/HomePage/PracticesIcons/litigation.png",
    },
  ],
};

export const locations = {
  image: "/assets/HomePage/Locations.jpg",
  title: "我们的办事处",
  content: `MT Law 在美国和中国设有办事处。
    我们在美国的办公室位于莱克星顿、波士顿、纽约、旧金山和洛杉矶。
    我们的中国办公室位于北京。`,
  button_content: "联系我们",
};

export const location_data = [
  {
    id: 0,
    title: "MTLaw 莱克星顿",
    content: `430 Bedford Street, Suite 200, Lexington, MA 02420, USA`,
  },
  {
    id: 1,
    title: "MTLaw 波士顿",
    content: "50 Milk Street 14th Floor, Boston, MA 02109, USA",
  },
  {
    id: 2,
    title: "MTLaw 纽约",
    content: "1270 Avenue of the Americas Suite 1800, New York, NY 10020, USA",
  },
  {
    id: 3,
    title: "MTLaw 洛杉矶",
    content: "201 South Lake Avenue Suite 701, Pasadena, CA 91101, USA",
  },
  {
    id: 4,
    title: "MTLaw 旧金山",
    content: "2901 Tasman Drive Suite 218, Santa Clara, CA 95054, USA",
  },
  {
    id: 5,
    title: "MTLaw 北京",
    content: "建国门外大街1号 中国国际大厦，2222室，北京，中国 100004",
  },
];

export const facts = [
  {
    big_text: "1000+",
    small_text: "企业客户",
  },

  {
    big_text: "20,000+",
    small_text: "个人客户",
  },
  {
    big_text: "30,000+",
    small_text: "案件和交易",
  },
];

export const case_studies = [
  {
    text: `XX 博士在一个非常复杂的科学领域拥有专业知识。 
        尽管他的研究需要大量的技术技能和经验， 
        但对美国的正面影响是间接的， 
        很难解释。不幸的是，XX 博士的雇主无法 
        赞助他的移民申请，因此他必须迅速申请 
        以避免失去永久居留权。`,
  },
  {
    text: `王先生（化名）和他的合伙人在一个美国州成立了一家饮料店。王先生选择了一个相当受欢迎的标志，并计划向美国商标局申请注册；但是，已经存在一个相似的商标。由于客户喜欢这个标志，王先生希望能够继续使用它。`,
  },
  {
    text: `田氏家族来自中国，他们希望有一个好的律师来协助他们在马萨诸塞州莱克星顿购买商业地产。一位朋友推荐了MT Law LLC。在听取了田氏家族的目标后，MT Law 及时协助他们完成了商业地产的购买。尽管田氏家族住在国外，MT Law 还是以专业且具成本效益的方式促成了这一过程。`,
  },
  {
    text: `MT Law 为一家知名的美国教育公司在与中国当地公司的商标、版权和域名纠纷中提供了建议。`,
  },
];

export const our_mission = {
  title: "高效且有效地解决您的法律需求",
  content: `当您与 MT Law LLC 合作时，您将拥有一家顶级全方位服务律师事务所在您的身边。拥有超过50名团队成员、五个全球办事处，并能说五种不同的语言，我们在多个行业、学科和地理区域拥有广泛的知识和经验。`,
};

export const our_team_introduction = {
  title: "我们的团队",
  content: `我们的团队致力于通过为您的具体情况量身定制的高质量代理，来满足您的法律需求。无论您的法律状况如何，我们都会作为一个团队来有效地解决您的担忧。让我们帮助您找到法律需求的解决方案。立即致电800-345-1899开始。我们希望将您加入我们卓越记录的行列。`,
  image: "/assets/HomePage/HomeTeam.jpg",
  color: "#00467f",
};
